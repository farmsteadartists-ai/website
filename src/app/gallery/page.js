...
export default function GalleryPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Build medium options from data
  ...
  useEffect(() => {
    ...
    if (medium !== 'All') {
      results = results.filter(a =>
        a.medium && a.medium.toLowerCase().includes(medium.toLowerCase())
      )
    }
    if (maxPrice) {
      results = results.filter(a => a.price && a.price <= parseFloat(maxPrice))
    }
    if (maxWidth) {
      results = results.filter(a => a.width_in && a.width_in <= parseFloat(maxWidth))
    }
    if (maxHeight) {
      results = results.filter(a => a.height_in && a.height_in <= parseFloat(maxHeight))
    }
  ...
  return (
    <section className="py-14 px-6 md:px-16 bg-cream-100 min-h-screen">
      ...
      <p className="text-gray-500 font-light mb-8 max-w-lg">
        Original artwork by Farmstead Artists members. Contact an artist directly to inquire about a piece.
      </p>
      ...
      <div className="bg-white rounded-xl p-5 border border-black/[0.04] shadow-sm mb-8">
        ...
        {hasFilters && (
          <button
            ...
            className="mt-4 text-sm text-sage-600 underline font-medium"
          >
            Clear all filters
          </button>
        )}
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20">
          ...
          <button onClick={clearFilters} className="mt-4 text-sage-600 underline text-sm">
            Clear filters
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filtered.map(work => (
            ...
            >
              ...
              {/* Details */}
              <div className="p-4">
                <h3 className="font-serif font-semibold text-sage-700 text-sm leading-tight truncate">
                  {work.title}
                </h3>
                {work.medium && (
                  <p className="text-xs text-gray-400 font-light mt-0.5">{work.medium}</p>
                )}
                {(work.width_in || work.size) && (
                  <p className="text-xs text-gray-400 font-light">
                    {work.width_in && work.height_in
                      ? `${work.width_in} × ${work.height_in} in`
                      : work.size}
                  </p>
                )}
                ...
                {work.artists && (
                  <Link
                    href={`/artists/${work.artists.slug}`}
